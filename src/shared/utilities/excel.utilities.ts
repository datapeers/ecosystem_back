...
const buildWorkbookBuffer = (columns: Partial<Column>[], rows, format: TableExportFormats = TableExportFormats.xlsx) => {
...
async function getWorkbookBuffer(workbook: Workbook, format: TableExportFormats) {
    switch (format) {
        case TableExportFormats.csv: return workbook.csv.writeBuffer();
        case TableExportFormats.xlsx:  return workbook.xlsx.writeBuffer();
        default: return workbook.xlsx.writeBuffer();
    }
}

const dotNotate = (obj: any, target: any = {}, prefix: string = "") => {
    Object.keys(obj).map((key) => {
        if ( typeof(obj[key]) === "object" && obj[key] !== null && !(obj[key] instanceof Types.ObjectId) ) {
...
const createRow = (
    ...
    config.forEach((columnConfig) => {
      ...
      Object.entries(dotNotatedDocument).map(([k,v]) => {
        const fixedKey = k.split('.').filter(kv => isNaN(+kv)).join('.');
...
const applyCellFormat = (value: string | string[], args: TableColumn): string => {
...
//TODO: Optimize this for the exclusive case of excel download
const parseDocumentsToRows = ( documents: any[], config: TableColumn[] = [] ): string[][] => {
    const rows = documents.map(doc => createRow(doc, config));
    return rows.map(rowValue => config.map(columnConfig => applyCellFormat(rowValue[columnConfig.key], columnConfig)));
}
