...
const buildAggregationFromRequest = (request: PageRequest, lookups: any[] = [], project: any = null, defaultMatch = { deletedAt: null }, paginated: boolean = true, outputProjection: any = null) => {
    ...
    let output: any[];
    if (paginated) {
        output = [{ $facet: { documents: documentsFacet, totalRecords: [{ $count: "count" }] } }, { $unwind: { path: "$totalRecords", preserveNullAndEmptyArrays: true } }, { $project: { documents: "$documents", totalRecords: { $ifNull: ["$totalRecords.count", 0] } } }];
    } else {
...
function _buildRegex(field: string, value: any, matchMode: string) {
    switch (matchMode) {
        case "onlyContains": {
            return { $and: [{ [field]: value }, { $expr: { $eq: [{ $size: `$${field}` }, 1] } }] };
        }
        case "notOnlyContains": {
            return { $and: [{ [field]: value }, { $expr: { $ne: [{ $size: `$${field}` }, 1] } }] };
        }
                ...
                $expr: {
                    $anyElementTrue: {
                        $map: {
                            input: { $ifNull: [`$${field}`, []] },
                            as: "var",
                            in: { $regexMatch: { input: "$$var", regex: value, options: "i" } },
                        },
                    },
                },
            };
        ...
        case "notContains": {
            ...
            return { [field]: { $not: { $regex: `${value.replace(EscapeCharactersRegex, "\\$1")}`, $options: "i" } } };
        }
        case "contains": {
            return { [field]: { $regex: `${value.replace(EscapeCharactersRegex, "\\$1")}`, $options: "i" } };
        }
        case "startsWith": {
            return { [field]: { $regex: `^${value.replace(EscapeCharactersRegex, "\\$1")}`, $options: "i" } };
        }
