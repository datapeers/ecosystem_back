...
export class TableConfigResolver {
  ...
  async createTableConfig(@Args('createTableConfigInput') createTableConfigInput: CreateTableConfigInput) {
    const table = await this.tableService.findOne({ _id: createTableConfigInput.table });
    const columns = await this.tableService.resolveTableColumns(table);
    return this.tableConfigService.createFromTable(columns, createTableConfigInput);
  }

  @Mutation(() => TableConfig)
  updateTableConfig(
    @Args('updateTableConfigInput') updateTableConfigInput: UpdateTableConfigInput,
  ) {
    return this.tableConfigService.update(updateTableConfigInput._id, updateTableConfigInput);
  }
