...
export class TableService {
  ...
  async create(data: CreateTableInput): Promise<Table> {
    ...
    if(!createdTable) throw new InternalServerErrorException(`Failed to create table with locator ${data.locator}`);
    const initialConfigData = { name: "Tabla", table: createdTable.id };
    const columns = await this.resolveTableColumns(createdTable);
    const initialConfig = await this.tableConfigService.createFromTable(columns, initialConfigData);
    if(!initialConfig) throw new InternalServerErrorException(`Failed to create initial table configuration for table ${createdTable.id}`);
    return createdTable;
  }

  async findOne(filters: { _id?: string, locator?: string }): Promise<Table> {
    const table = await this.tableModel.findOne(filters);
    if(!table) throw new NotFoundException(`Couldn't find a table by the specified filters: ${filters}`);
    return table;
  }
  ...
  async resolveTableColumns(table: Table) {
    ...
    const columns = tableUtilities.convertFormToColumns(formComponents.components);
    return columns;
  }
}
